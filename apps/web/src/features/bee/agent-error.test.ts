...
import { beeSendFailureMessage, friendlyBeeErrorMessage } from './agent-error'
...
describe('Bee agent subscription recovery', () => {
  it('turns a Flue 402 into the truthful iOS recovery path', () => {
    const error = Object.assign(
      new Error('Flue API error 402: request failed'),
      {
        status: 402,
  ...
  it('keeps ordinary upstream failures as connection recovery', () => {
    const error = Object.assign(
      new Error('Flue API error 503: request failed'),
      {
        status: 503,
      },
    )
