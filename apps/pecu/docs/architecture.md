...
Web accounts can also link EOAs they hold. Linking needs a stored Sign-In with Ethereum challenge signed by that address. A web thread can pick a linked wallet; plans built for it carry `signer`, the browser wallet signs each exact persisted call, and Pecu settles every step from its own Base RPC. Crossmint never signs these plans. See `docs/47-pecu-linked-wallets.md` at the monorepo root.
...
`src/execution.ts` runs the steps as an Effect program and classifies every failure into one of four typed errors, so the agent decides the intent's fate from the tag rather than from message text. `StepFailed` (expiry before prepare or approve, a prepare error, a wrong-wallet record, an approval Crossmint rejected while the record still says `awaiting-approval`, or a Crossmint status of `failed`) and `Reverted` (a verified `UserOperationEvent` with `success = false`) are terminal: the exact step is marked `failed` and the intent moves to `failed`. `OutcomeUnknown` (any read, approve, or receipt-RPC error once a Crossmint transaction id exists) and `InclusionPending` keep the intent `executing`; the user is told nothing will be resubmitted and to send the same code again, and boot recovery re-drives the step. A transient error right after `approve()` therefore can no longer record a landed user operation as a failure.

Confirmation codes are six symbols from a 32-character alphabet without I, O, 0, or 1 (about 1.07 billion codes), and `intentForCode` is scoped to `(code hash, sender, conversation)` on top of the `UNIQUE(code_hash)` constraint.
...
`src/evm.ts` is the Worker-side client. It resolves token symbols through the Aero SDK's canonical Base token list, converts human amounts to base units with string arithmetic after reading the token's decimals, refuses transfers that exceed the sender's balance before asking the sandbox for a plan, and requires the returned plan's `account` to equal the sender's wallet. `validateEvmPlan` in `src/policy.ts` then pins the single returned call to the declared action: native transfers carry `0x` calldata and positive value, ERC-20 transfers and approvals carry exactly the `transfer(address,uint256)` or `approve(address,uint256)` selector with 64 bytes of arguments and zero value, revokes must encode a zero allowance, and contract calls need calldata whose selector is not an ERC-20, ERC-721, ERC-1155, or Aave `approveDelegation` transfer/approve function, so a model-authored call cannot bypass the token validators. Because contract-call calldata is authored by the model rather than an SDK, `requiresExplicitConfirmation` also keeps it out of YOLO: the preview always waits for the user's `confirm`. Intents record `family: "aero" | "stocks" | "evm" | "aave" | "deposit"` so both stores re-validate parameters with the right validator on load.
...
GPT-6 Sol uses medium reasoning; GPT-6 Luna uses low reasoning. `src/cloudflare/model-catalog.ts` registers both models explicitly because the pinned OpenCode snapshot predates their release. The Codex container allowlist must be deployed alongside the main Worker when changing these model IDs. Web and X Chat share this harness; other Bee clients use their own agent configuration.
