...
if (process.argv.includes("--local-boundary")) {
  ...
  const keychain = Bun.spawn(["security", "find-generic-password", "-s", "com.oddofrancesco.pecu.admin-token", "-w"], { stdout: "pipe", stderr: "pipe" });
  const token = (await new Response(keychain.stdout).text()).trim();
  if (await keychain.exited !== 0 || !token) throw new Error("Pecu admin token is unavailable");
  const path = process.argv.includes("--tools") ? "tools" : "probe";
  const response = await fetch(`https://pecu.oddofrancesco000.workers.dev/admin/opencode/${path}`, {
