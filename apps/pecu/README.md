...
Use `/polymarket QUESTION` to search Polymarket directly, or ask the agent about markets, prices, positions, activity, PnL, holders, rankings, sports or rewards. The agent has 52 public read tools backed by Gamma, CLOB and Data API v2. No API key is needed. `/polymarket help` lists the reads; `/polymarket read ENDPOINT JSON` calls one directly, for example `/polymarket read leaderboard {"limit":5}`. Wallet reads require the actual Polymarket proxy/deposit wallet, which may differ from the Pecu Base wallet.

The Effect service validates inputs and responses, retries transient GET failures at most twice, respects numeric Retry-After delays, and interrupts requests after 20 seconds. Responses retain source URLs, retrieval timestamps, and continuation arguments with the original filters. Missing values remain unavailable. No trading, signing or bridge writes are exposed.

`/polymarket research QUESTION` explicitly starts the optional Exa research flow; `/polymarket status` retrieves its saved run. Only this research path needs `EXA_API_KEY`. See [the endpoint and verification guide](../../docs/42-pecu-polymarket.md).
