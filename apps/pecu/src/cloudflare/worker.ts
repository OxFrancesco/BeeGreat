...
function durableObject(env: Cloudflare.Env): DurableObjectStub {
  return env.PECU.get(env.PECU.idFromName(objectName));
}
...
export class PecuDurableObject extends DurableObject<Cloudflare.Env> {
  ...
  override async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === "/internal/realtime/setup" && request.method === "POST") {
        const webhookUrl = request.headers.get("X-Pecu-Webhook-Url");
  ...
  private async ensureTransport(): Promise<XChatTransport> {
    if (!this.agent || !this.xAccessToken) throw new Error("Pecu XChat runtime is not ready");
...
export default {
  async fetch(request: Request, env: Cloudflare.Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/x/webhook") {
      ...
      if (request.method === "POST") {
        ...
        const queued = await durableObject(env).fetch(new Request("https://pecu.internal/internal/activity", {
          method: "POST",
    ...
    if (internalPath === "/internal/realtime/setup") {
      internalRequest.headers.set("X-Pecu-Webhook-Url", new URL("/x/webhook", request.url).toString());
    }
  ...
  scheduled(_controller: ScheduledController, env: Cloudflare.Env, ctx: ExecutionContext): void {
    ctx.waitUntil((async () => {
      const response = await durableObject(env).fetch(new Request("https://pecu.internal/internal/cron", {
        method: "POST",
      }));
