...
const emptyToUndefined = z.string().optional().transform(value => value === "" ? undefined : value);
...
const envSchema = z.object({
  ...
  CHAT_BOT_USER_ID: emptyToUndefined.pipe(z.string().regex(/^\d+$/).optional()),
...
export function loadConfig(env: Record<string, string | undefined> = Bun.env): Config {
  ...
  return {
    ...
    xurlApp: value.XURL_APP,
    xurlUsername: value.XURL_USERNAME,
    chatBotUserId: value.CHAT_BOT_USER_ID,
