...
describe("X Chat WASM in Workerd", () => {
  test("initializes the X Chat and Juicebox WASM modules", async () => {
    ...
    const worker = Bun.spawn({
      ...
      env: {
        ...process.env,
        WRANGLER_REGISTRY_PATH: `/tmp/pecu-wrangler-test-${process.pid}/registry`,
    ...
    try {
      ...
      expect(JSON.parse(modelBody)).toEqual({ requests: 4, fallback: [
        { path: "/api/v1/chat/completions", authorized: true, model: "openai/gpt-6-sol", effort: "medium", provider: { only: ["openai"] } },
        { path: "/api/v1/chat/completions", authorized: true, model: "openai/gpt-6-luna", effort: "low", provider: { only: ["openai"] } },
      ] });
