...
- CL deposit options are `--price-lower`, `--price-upper`, `--tick-lower`, `--tick-upper`, and `--initial-price`. Their values depend on the actual pool. Read balances and pool data first, then recommend affordable amounts and concrete bounds for the user to accept or adjust. State the price convention and range tradeoff. Do not invent market data or prepare a transaction from unaccepted suggested parameters, including when YOLO is on. Basic pools reject CL-only price and tick flags.
...
The profile at `/profile` (`src/safe-profile.ts`, `apps/stocks/src/components/profile/`) groups Safes into organizations owned by the Pecu sender and keeps one shared proposal queue per Safe address. Store a browser-wallet signature only after recovering it to a current owner at the current nonce. Profile actions that sign run in the `profile:USER:SENDER` conversation so YOLO never applies. Pecu wallet execution must use exactly the threshold of signatures, with the executor's own approved-hash entry chosen first. Never mark a browser-reported execution as executed without an `ExecutionSuccess` receipt for the proposal hash. Removing an organization or Safe removes only the local record. Keep `safe_queue` and the profile in step when the queue shape changes.
...
Accounts can link EOAs they hold (`src/linked-wallets.ts`, `src/linked-execution.ts`, contract in `src/linked-wallet-contract.ts`). Linking stores a Sign-In with Ethereum challenge bound to the sender, the page origin and Base, deletes it on first use, and accepts only an ecrecover signature from the challenged address. Never link a wallet from an unsigned address or accept a contract signature. See `docs/47-pecu-linked-wallets.md` at the monorepo root.

A web thread can pick a linked wallet. Plans built for it are saved with `signer`, and the Crossmint signer must never run them: keep them out of YOLO, `/confirm`, X Chat and boot recovery. The web card hands out one exact persisted call at a time and a step succeeds only after Pecu's own Base RPC shows that call from that wallet with a successful canonical receipt. Keep the unlink guard for executing plans, and keep the profile Safe form, owner labels and chat picker reading the same linked list.
