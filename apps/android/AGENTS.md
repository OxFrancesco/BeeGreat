...
- `app`: entry point, `AppContainer` (manual DI), auth gate, tab shell, and
  the feature packages `bee` (chat), `goals`, `hive`, `mind`, `healthy`,
  `voice`, `profile`, `web3`, `nfc`.
...
- `core/flue` (pure JVM): the `@flue/sdk` conversation client. Wire models,
  the chunk reducer, HTTP send/history/abort, the Durable Streams reader
  (JSON catch-up, SSE tail, long-poll), `ConversationObserver` (dedup by
  position, incarnation resync, stale watchdog), and the optimistic
  `AgentSession` from `@flue/react`. Tests use MockWebServer.
- `core/contract` (pure JVM): ports of `tool-presentation` and `chat-sync`.
  beeui parsing, identifier scrubbing, follow-ups, tool copy, transcript
  merge, the sync queue, and the small pure helpers screens share (health
  keys, bookmark URLs, Google Workspace disclosure).

## Shared fixtures

`packages/tool-presentation/fixtures/beeui.json` is generated by
`bun packages/tool-presentation/fixtures/generate.ts` from the TypeScript
implementation and asserted by both `bun test` (`fixtures.test.ts`) and
JUnit (`SharedFixturesTest`). A contract change fails one side until the
other follows. Regenerate, never hand-edit.

## Release

`./gradlew :app:assembleRelease` minifies with R8. `proguard-rules.pro`
carries keep rules for three things that only break in release builds:
Clerk's Retrofit interfaces (generic signatures), Convex's JNA bridge
(classes resolved by name from Rust), and our kotlinx.serialization types.
When a release build crashes on launch and the debug build does not, retrace
first:

```sh
/opt/homebrew/share/android-commandlinetools/cmdline-tools/latest/bin/retrace \
  app/build/outputs/mapping/release/mapping.txt crash.txt
```

Signing reads `RELEASE_STORE_FILE`, `RELEASE_STORE_PASSWORD`,
`RELEASE_KEY_ALIAS`, `RELEASE_KEY_PASSWORD` from `local.properties` or the
environment and falls back to the debug key.

## What is not ported, on purpose

Apple Sign-In, the iOS Live Activity (a foreground-service notification
stands in), the paywall and RevenueCat rows (the Expo app gates only iOS),
SF Symbols, the glass effect, and the `*.web.tsx` files. Everything else in
`apps/mobile/src/app` has a Compose counterpart.
