...
const transaction = z.object({
  ...
  data: z.string().regex(/^0x(?:[0-9a-fA-F]{2}){4,}$/), value: z.string().regex(/^\d+$/),
});
const envelope = z.object({ data: z.unknown(), warnings: z.array(z.object({ level: z.string(), message: z.string() }).passthrough()).optional() }).passthrough();
export type AavePlan = { parameters: AaveParameters; calls: PlannedCall[]; preview: string; details: unknown };

export class AaveService {
  constructor(private readonly request: typeof fetch = fetch) {}
  async call(name: string, input: Record<string, unknown>, wallet: string): Promise<unknown> {
    const tool = tools.find((item) => item.name === name);
    if (!tool) throw new Error("That Aave tool is not supported by BasedBot.");
    ...
    return this.rpc(name, schema.parse(args));
  }
  private async rpc(name: string, args: unknown): Promise<unknown> {
    ...
    const parsed = z.object({ error: z.unknown().optional(), result: z.object({ isError: z.boolean().optional(), structuredContent: z.unknown().optional(), content: z.array(z.object({ type: z.string(), text: z.string().optional() })).optional() }).optional() }).parse(await response.json());
  ...
  async propose(input: Record<string, unknown>, wallet: string): Promise<AavePlan> {
    if (input.chainId !== undefined && input.chainId !== 8453 || input.version !== undefined && input.version !== "v3") throw new Error("Aave transactions in BasedBot use Base v3 only.");
    ...
    const raw = z.object({ __typename: z.string() }).passthrough().parse(built.data);
