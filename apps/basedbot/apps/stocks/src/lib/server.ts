...
export async function identity() {
  ...
  if (!userId) throw new Error("Sign in with X to use your BasedBot wallet.");
...
export async function agentRequest(
  ...
  const binding: unknown = Reflect.get(env, "BASED_BOT");
  if (
    !binding ||
  ...
  return binding.fetch(`https://basedbot.internal/${path}`, {
    method: "POST",
