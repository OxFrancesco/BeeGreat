...
```sh
SAFE_TEST_CONFIG="$PWD/.env.safe-testnet.json" bun run --cwd packages/evm test:safe:sponsored
```
...
The Safe form lists the account's [linked wallets](47-pecu-linked-wallets.md) as owner choices, and linked names label owners, approvals and spending-limit delegates. Connect wallet offers EIP-6963 extensions and WalletConnect; a queued transaction whose owner is an unconnected linked wallet offers to connect it.
