...
| Generated UI and confirmed actions         | [05 – Voice Agent](05-voice-agent.md), docs 02/08                                                                 | FRA-445, FRA-456, FRA-461                                  | Bee emits the editable first-focus preview; Expo and web validate/render it, while iMessage validates and projects the same vocabulary into styled text and native link cards. All three submit the same atomic/idempotent first-focus and Highlight actions.                                                                                                                      | **Shared behavior implemented; protocol versioning remains partial.** FRA-456 still owns an explicit versioned wire contract and broader portability guarantees.                                                                  |
...
| Agent orchestration and power-ups          | Docs 03, 05, 07                                                                                                   | FRA-407, FRA-422; FRA-405 is a completed pattern donor     | Bee delegates goals work to a specialist and dynamically loads opt-in power-up subagents. Web3 provides guarded Crossmint wallet tools and profile toggles.                                                                                                                       | **Implemented specific capability, missing generic connector contract.** Web3 is orphaned from Linear. FRA-422 should not be marked complete by it; auth/retries/observability/fixtures for general connectors remain open. |
...
| iMessage                                   | Docs 03; generated UI intent in docs 05                                                                           | FRA-445                                                    | Spectrum Cloud bridge, allowlisted identity mapping, shared active conversations, the full Bee runtime, text/voice/image ingress, complete generated-UI projection, native link cards, confirmed first-focus/Highlight actions, activity reaction, and server-confirmed confetti are implemented.                                                                                                                                      | **Behavior parity implemented.** A dedicated BeeGreat Messages extension is not required for parity because native link cards plus accessible text projection cover the current vocabulary; revisit only for future interaction types that cannot project safely.                                                  |
| Web and iPad                               | Docs 02, 03, 05, 07                                                                                               | FRA-456 and FRA-457 indirectly                             | TanStack web implements the mobile product surface with shared Clerk/Convex/Flue auth, Bee/voice/threads, generated UI, Goals/Projects/Tasks, Hive, connections, power-ups, and settings. Expo declares iPad-capable dependencies but uses phone-oriented portrait configuration. | **Web feature twin implemented and audited.** Native-only haptics and ActivityKit use browser-appropriate feedback; iPad-specific layouts remain separate work.                                                             |
...
5. **MVP composition:** the first-focus baseline remains narrow, but the founder
   ...
   generated bees, handles/social, memory retrieval, and iPad-specific parity remain deferred.
...
8. **Generated UI:** the useful mobile v0 now renders and confirms on web;
   FRA-456 still owns protocol versioning and iMessage fallback semantics.
9. **Web parity:** TanStack now matches every connected mobile product workflow
   and shares its Clerk, Convex, Flue, thread, and generated-UI contracts.
...
- The power-up registry and Web3 wallet capability.
