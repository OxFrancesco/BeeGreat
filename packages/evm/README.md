...
```sh
bun install
bun run --cwd packages/evm cli discover
bun run --cwd packages/evm cli tui
bun run --cwd packages/evm cli mcp
```

Build with `bun run --cwd packages/evm build`, then run `bun packages/evm/dist/cli.js discover`. `discover` publishes input and output schemas, errors and execution conventions. Commands accept `--input`, `--file` or `--stdin`. Results are JSON; watches emit NDJSON. Errors include a stable code and exit with status 1. MCP uses stdio and the same catalog.

## Wallets

```sh
bun run --cwd packages/evm cli wallet connect --browser --chain 8453 --name main
bun run --cwd packages/evm cli wallet connect --chain 8453 --name mobile
bun run --cwd packages/evm cli wallet connect --smart --chain 8453 --name smart
bun run --cwd packages/evm cli wallet status
bun run --cwd packages/evm cli wallet select --name main
bun run --cwd packages/evm cli wallet disconnect --name main
```
...
```sh
bun run --cwd packages/evm cli read --input '{"chainId":8453,"address":"0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913","signatures":["function decimals() view returns (uint8)"],"functionName":"decimals"}'
bun run --cwd packages/evm cli prepare-call --file transaction.json
bun run --cwd packages/evm cli execute --input '{"id":"PLAN_ID"}' --yolo
bun run --cwd packages/evm cli status --input '{"id":"PLAN_ID"}'
```
...
```sh
bun run --cwd packages/evm typecheck
bun run --cwd packages/evm lint
bun run --cwd packages/evm test
bun run --cwd packages/evm build
bun run --cwd packages/evm test:e2e
```
