...
const agentIdentities = {
  "bee": { bindingName: "FLUE_BEE_AGENT", className: "FlueBeeAgent" },
};
...
const agentExtension0 = resolveCloudflareExtension(agentModules["bee"], "bee", 'Agent');
const FlueBeeAgent = class FlueBeeAgent extends agentExtension0.base(Agent) {
  constructor(ctx, env) {
    const prepared = cloudflareAgents.prepare({ storage: ctx.storage, className: "FlueBeeAgent", agentName: "bee" });
    super(ctx, env);
...
const WrappedFlueBeeAgent = agentExtension0.wrap(FlueBeeAgent);
export { WrappedFlueBeeAgent as FlueBeeAgent };
