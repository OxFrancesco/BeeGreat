...
async function seed(t: ReturnType<typeof convexTest>, subscribed = true) {
  if (subscribed) await t.mutation(internal.subscriptions.applyRevenueCatRestSnapshot, { userId: "user_delivery", checkedAt: Date.now(), active: true, productId: REVENUECAT_MONTHLY_PRODUCT_ID, environment: "PRODUCTION", periodStartedAt: Date.now() - 1000, expiresAt: Date.now() + 3600000 })
...
test('lost admission response retries the same payload and key then saves the admission receipt', async () => {
  ...
  await t.action(ctx => deliverBeennectorForId(ctx, id))
  ...
  await t.action(ctx => deliverBeennectorForId(ctx, id)); expect(requests).toHaveLength(2)
})

test('subscription outages retain delivery and disconnect cancels it before admission', async () => {
  const t = convexTest(schema, modules); const { id, connectionId } = await seed(t, false)
