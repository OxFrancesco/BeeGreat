...
- RPC comes from `AERO_VERIFY_RPC` if set, else an ambient `SUGAR_RPC_URI_8453`, else `https://base-rpc.publicnode.com`; the SDK's public default is too throttled for the quote path and a dedicated endpoint makes runs faster (the TUI raises scan concurrency when `SUGAR_RPC_URI_8453` is set). On publicnode the env caps `SUGAR_POOL_PAGINATION_MAX_SIZE_8453` at 75 and `SUGAR_PRICE_BATCH_SIZE_8453` at 8 because larger multicall batches exceed its call cap.
