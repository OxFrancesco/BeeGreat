...
- `stocks-list` shows the catalog and available price results.
...
- For buys, the wallet holds USDC (the runner orders swaps first). The `--cash 0.5` rebalance preview also requires at least 0.5 USDC and a wallet address, even without broadcasting.

- **List.** Run `scripts/aero stocks list`. Array of ten markets with `symbol`, `address`, and `price_usdc`. Check price values separately: catalog count alone does not prove live pricing succeeded.
...
- **Index lifecycle.** Run `scripts/aero index create --name aero-verify --allocations NVDAc=50,AAPLc=50`, then `scripts/aero index list`, `scripts/aero index show --name aero-verify`, `scripts/aero index update --name aero-verify --allocations NVDAc=100`. Each prints the index JSON or list; `show` and `update` must round-trip the allocation string.
- **Rebalance preview.** Run `scripts/aero index rebalance --name aero-verify --cash 0.5 --dry-run`. The plan JSON contains a `trades` array. Run this before deleting the saved index. Then run `scripts/aero index delete --name aero-verify`; it prints `Deleted aero-verify. Wallet holdings unchanged.` and the entry disappears from `index list`.
- **Runner.** `bun scripts/verify.ts --mode full --only swap,stocks,sweep` for the buy and sell; `--only indices` for the local index steps plus the rebalance dry-run, provided the wallet already holds the preview's required USDC. `swap` is included because the buy needs USDC in the wallet, and `sweep` returns the balances to ETH afterwards.
