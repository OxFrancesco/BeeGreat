...
RPC resolution: `AERO_VERIFY_RPC` wins, then an ambient `SUGAR_RPC_URI_8453`, then `https://base-rpc.publicnode.com`. Reports show only the kind (`verify-override`, `user-env`, `publicnode-default`), never the URL.

publicnode is reads-only: it answers `eth_getTransactionReceipt` with a 403 `Archive requests require a personal token`, so `full` and `sweep` refuse at preflight when it is the resolved endpoint. It remains the fallback for `reads` and `dry-run`, where it completes the ETH to USDC quote in about 1 minute 40 seconds through 429 retries.
...
Two failure shapes reconcile themselves through that same path. When the endpoint drops `eth_getTransactionReceipt`, the CLI's send can confirm on chain while its receipt wait fails with `Execution outcome unknown; resume <id> to check <hash>`. The CLI can also die earlier, on `Local transaction preparation failed before broadcast`, which leaves the unbroadcast step `ready` and the journal `active`. In both shapes the runner finds the journal the run just created, confirms no step is still `submitting`, drives `executions resume` through expect, collects every hash the journal holds, fetches those receipts with viem, and marks the step `flaky` (its check still runs, so downstream steps are not blocked). A step left `submitting` is never auto-resumed, because its broadcast outcome is genuinely unknown, and neither is a journal that predates the run. If the receipt reverted or the reconcile fails, the step is `fail`, the run aborts, and the record prints the manual resume or cancel command.
